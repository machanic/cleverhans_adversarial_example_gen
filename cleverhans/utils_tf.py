...
def projection(eta, ord, eps):
    ...
    reduc_ind = list(xrange(1, len(eta.get_shape())))
    avoid_zero_div = 1e-12
    eta_shape_ = tf.shape(eta)
    # eta_shape_ = eta.get_shape().as_list()
    eta_ = tf.reshape(eta, (eta_shape_[0], -1))
    if ord == np.inf:
        eta_ = clip_by_value(eta_, -eps, eps)
    elif ord == 1:
        eta_ = eta_ * tf.expand_dims(tf.minimum(tf.div(eps, tf.norm(eta_, axis=1, ord=1) + avoid_zero_div), 1.), axis=1)
    elif ord == 2:
        eta_ = eta_ * tf.expand_dims(tf.minimum(tf.div(eps, tf.norm(eta_, axis=1, ord=2) + avoid_zero_div), 1.), axis=1)
        #     # avoid_zero_div must go inside sqrt to avoid a divide by zero
        #     # in the gradient through this operation
        #     norm = tf.sqrt(tf.maximum(avoid_zero_div,
        #                               reduce_sum(tf.square(eta),
        ...
        #     eta = eta * factor
        # if ord!=2:
    eta = tf.reshape(eta_, eta_shape_)
    return eta
...
def look_for_target_otherthan_gt(gt_label_onehot, class_number):
    # gt_label_tensor shape : N, class_number
    reverse_gt_label_onehot = 1 - gt_label_onehot  # reverse version
    reverse_gt_label_onehot = tf.cast(reverse_gt_label_onehot, tf.float32)
    sampled_val = tf.random.multinomial(reverse_gt_label_onehot, 1)  # [batch_size, 1]
    sampled_val = tf.squeeze(sampled_val)
    sampled_val = tf.one_hot(sampled_val, depth=class_number, on_value=1.0, off_value=0.0)  # [batch, class_number]
    return sampled_val
