import numpy as np
from cleverhans.attacks import FastGradientMethod, CarliniWagnerL2, MadryEtAl, SaliencyMapMethod, DeepFool, \
    ElasticNetMethod, BasicIterativeMethod,MomentumIterativeMethod,SpatialTransformationMethod,VirtualAdversarialMethod,\
    SPSA, LBFGS, CleanIdentity,NewtonFool, Noise,MaxConfidence,Semantic


# dataset config
ROOT_DATA_DIR = "/home1/machen/dataset"
# ROOT_DATA_DIR = "G:/dataset/"
CIFAR10_SOURCE_DATA_DIR = "{}/CIFAR-10/cifar-10-batches-py".format(ROOT_DATA_DIR)
...
AWA2_TRAIN_CLASSES = ["killer+whale", "beaver", "dalmatian", "persian+cat", "german+shepherd", "siamese+cat", "skunk", "mole", "tiger", "hippopotamus", "leopard", "spider+monkey", "elephant", "gorilla", "ox", "chimpanzee", "hamster", "fox", "squirrel", "rabbit", "wolf", "chihuahua", "weasel", "otter", "buffalo", "zebra", "giant+panda", "pig", "lion", "polar+bear", "collie", "cow", "deer", "mouse", "humpback+whale", "antelope", "grizzly+bear", "rhinoceros", "raccoon", "moose"]
AWA2_TEST_CLASSES = ["sheep", "dolphin", "bat", "seal", "blue+whale", "rat", "horse", "walrus", "giraffe", "bobcat"]

AWA2_SOURCE_DATA_DIR = "{}/AwA2/".format(ROOT_DATA_DIR)
AWA2_OUTPUT_DATA_DIR = "{}/AwA2/adversarial_images".format(ROOT_DATA_DIR)
AWA2_MODEL_STORE_PATH = "{}/AwA2/tf_trained_model".format(ROOT_DATA_DIR)

CUB_SOURCE_DATA_DIR =  "{}/CUB200-2011/".format(ROOT_DATA_DIR)
...
ATTACKERS = {"FGSM": FastGradientMethod, "CW_L2": CarliniWagnerL2,
             "PGD_L_infinity": MadryEtAl, "PGD_L2": MadryEtAl,
             "jsma": SaliencyMapMethod,
             "deep_fool_L2": DeepFool, "deep_fool_L_infinity": DeepFool,
             "BIM_L2": BasicIterativeMethod, "BIM_L1":BasicIterativeMethod, "BIM_L_infinity": BasicIterativeMethod,
             "EAD": ElasticNetMethod,
             "MI_FGSM_L_infinity": MomentumIterativeMethod, "MI_FGSM_L2":MomentumIterativeMethod,
             "spatial_transform": SpatialTransformationMethod, "VAT": VirtualAdversarialMethod,
             "SPSA":SPSA, "LBFGS":LBFGS,
...
NEED_TARGETED_Y = ["SPSA", "LBFGS"]

# 下面的方法一律不提供ground truth label y，如果要提供，需要每次传入一个参数y
UNTARGETED_ATTACKER_PARAM = {
    "FGSM": {
        'eps': 0.3,
        'clip_min': -2.7,
        'clip_max': 2.7
        # 'y_target': True ==> targeted attack  # default is untargeted attack
    },
    "CW_L2": {
    ...
    "PGD_L_infinity": {'batch_size': BATCH_SIZE, "rand_init":True, "rand_minmax": 0.3,
            'clip_min': -3., 'clip_max': 3., "eps": 0.3, "alpha": 0.01, "nb_iter": 40, "ord": np.inf},
    "PGD_L2": {'batch_size': BATCH_SIZE, "rand_init":True, "rand_minmax": 0.3,
    ...
    "MI_FGSM_L_infinity": {"ord":np.inf, "nb_iter":40, "decay_factor":1.0, "eps_iter":0.1, "clip_min":-3., "clip_max":3., } , # inception v3 decay_factor = 16
    # 成功率低：VAT: 0.4367， MI_FGSM_L2, BIM_L2:0.165, deep_fool_L_infinity
    "VAT": {"nb_iter":20, "clip_min":-3., "clip_max":3., "eps":6.0, "xi":1e-6},  # 测试得到的精确率偏低
    "MI_FGSM_L2": {"ord":2, "eps":0.3, "nb_iter":40, "decay_factor":1.0, "eps_iter":0.1, "clip_min":-3., "clip_max":3., },
    ...
    "BIM_L_infinity" :{"ord":np.inf, "rand_init":False, "rand_minmax":0, "nb_iter":40, "clip_min":-3., "clip_max":3.},
    "BIM_L1" :{"ord":1, "rand_init":False, "rand_minmax":0, "nb_iter":40, "clip_min":-3., "clip_max":3.},
