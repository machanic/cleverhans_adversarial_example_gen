...
class SPSA(Attack):
    ...
    def generate(self,
        ...
        if batch_size is not None:
            ...
            # spsa_samples = batch_size FIXME cause error exit the next line!
...
def projected_optimization(loss_fn,
    ...
    def loop_body(i, perturbation, flat_optim_state):
        ...
        def wrapped_loss_fn(x):
            label_repeat = tf.tile(label, multiples=[x.get_shape().as_list()[0], 1])
            return loss_fn(input_image + x, label_repeat)
